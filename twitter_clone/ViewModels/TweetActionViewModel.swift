...
class TweetActionViewModel: ObservableObject{
    ...
    func likeTweet(){
        ...
        COLLECTION_TWEETS.document(tweet.id).updateData(["likes" : tweet.likes + 1]) {
            _ in
            tweetLikeRef.document(uid).setData([:]){
                _ in
                userLikesRef.document(self.tweet.id).setData([:]){
                    _ in
                    self.didLike = true
                }
            }
    ...
    func unlikeTweet(){
        ...
        COLLECTION_TWEETS.document(tweet.id).updateData(["likes" : tweet.likes - 1]) {
            _ in
            tweetLikeRef.document(uid).delete{
                _ in
                userLikesRef.document(self.tweet.id).delete{
                    _ in
                    self.didLike = false
                }
            }
